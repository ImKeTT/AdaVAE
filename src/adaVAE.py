...
def compute_loss(device, model, input_tokens, mask, x_tokens, label_onehot, loss_fn, beta, use_adv_loss):
    """
    ...
    outputs = model(input_ids=input_tokens, attention_mask=mask, label_onehot=label_onehot, from_mean=True)
    logits = outputs[0]
    ...
    if mask is not None:
        ...
        x_tokens = x_tokens.masked_select(mask)
...
def tokenize(texts, tokenizer, device):
    ...
    x_mask = x_tokenized['attention_mask'][:, 1:].to(device)
    ## true_in, true_mask, dec/enc_in, dec/enc_mask
    return x_ids, x_mask, input_ids, attention_mask

def train_step(device, model, optimizer, x_tokens, input_tokens, mask, label_onehot, loss_fn, beta, use_adv_loss, model_type):
    # output = []
    ...
    optimizer.zero_grad()
    loss, ce_loss, reg_loss = compute_loss(device, model, input_tokens, mask, x_tokens, label_onehot, loss_fn,
                                          beta, use_adv_loss)
    with amp.scale_loss(loss, optimizer) as scaled_loss:
        scaled_loss.backward()
        torch.nn.utils.clip_grad_norm_(amp.master_params(optimizer), 5.0)  # max_grad_norm=1.0
    ...
    while num_iters < args.iterations:
        ...
        with tqdm(total=len(train_loader)) as pbar:
            for i, data_dict in enumerate(train_loader):
                x_ids, x_mask, input_ids, attention_mask = tokenize(data_dict['x'], tokenizer, device)
                ...
                loss, ce_loss, reg_loss = train_step(device, AdaVAE, optimizer, x_ids, input_ids, x_mask,
...
if __name__=="__main__":
    args = parser.parse_args('train --batch-sizes 5 --add_attn --label_cond'.split())
    train(args)
